...
fn mt19937_get(mut state: MTState) -> UInt64:
    """Generate next MT19937 random value."""
    if state.idx >= MT_N:
        var i: Int = 0
        while i < MT_N - MT_M:
            var y = (state.state[i] & MT_UPPER_MASK) | (state.state[i + 1] & MT_LOWER_MASK)
            var idx = i + MT_M
            state.state.store(i, state.state[idx] ^ (y >> 1) ^ MT_MATRIX_A * (y & 1))
            i += 1
        ...
        y = (state.state[MT_N - 1] & MT_UPPER_MASK) | (state.state[0] & MT_LOWER_MASK)
        var idx = MT_M - 1
        state.state.store(MT_N - 1, state.state[idx] ^ (y >> 1) ^ MT_MATRIX_A * (y & 1))
...
fn mt19937_get_double(mut state: MTState) -> Float64:
...
struct RNG(Copyable, Movable):
    ...
    fn __init__(out self, seed: UInt64 = 5489):
        self.state = MTState()
        self.seed_val = seed
        self.name = "mt19937"
    ...
    fn uniform_int(mut self, n: Int) -> Int:
