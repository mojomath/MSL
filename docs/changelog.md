...
- Rewrote every README code example against the current API; all eight now compile and run cleanly (previously several referenced deprecated/removed stdlib symbols).
- Expanded the README's module Overview, API reference, and Status tables to include `msl.linalg`, `msl.poly`, and `msl.statistics`, and corrected the test count (330, up from the previously-stated ~280).
