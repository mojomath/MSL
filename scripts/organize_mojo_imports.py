...
Usage:
    python3 scripts/organize_mojo_imports.py msl
...
def find_package_root(file_path: Path) -> Path | None:
    ...
    root, e.g. `msl/`). Returns None if `file_path`'s own directory
...
def resolve_relative_module(module: str, file_path: Path, package_root: Path) -> str:
    ...
    `msl.`-rooted form, based on where `file_path` sits under
    `package_root`. Non-relative modules are returned unchanged.

    MSL convention (standardized 2026-08-28): all internal imports are
    absolute (`from msl.integration.qk21 import qk21`), never relative
    (`from .qk21 import qk21` / `from ..core.errno import X`).
    """
...
def import_group(module: str) -> int:
    ...
    if root == "msl":
        return 3
    if root == "mojoblas":
        # mojoBLAS is a separate external dependency (msl.blas wraps it),
        # not part of MSL's own package tree — group with other externals,
        # not under the "MSL" banner.
        return 1
    return 1
...
def organize_file(path: Path, *, keep_unused: bool) -> str | None:
    ...
    # MSL convention: internal imports are absolute (`msl.x`), never
    # relative (`.x` / `..x`) — resolve relative modules if we can
